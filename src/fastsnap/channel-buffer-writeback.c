...
/* fastsnap addition: a read-only view over a caller-owned buffer. See the
 * header for why this exists instead of a patch to io/channel-buffer.c. */
QIOChannelBufferWriteback*
qio_channel_buffer_writeback_new_reader(uint8_t* buf, size_t usage)
{
    ...
    ioc->writeback_buf = NULL;
    ioc->writeback_buf_capacity = 0;
    ioc->writeback_buf_usage = NULL;
...
static void qio_channel_buffer_writeback_finalize(Object* obj)
{
    ...
    /* fastsnap: assert moved inside the guard -- a reader channel has no
     * write-back target and a zero capacity, which tripped this. */
